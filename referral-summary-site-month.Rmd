...
wq_all_processed <- wq_all_data %>%
  mutate(referral_type = case_when(str_detect(WORKQUEUE_NAME, "REFERRAL TO") ~ "TO WQ",
...
wq_all_processed <- wq_all_processed %>%
  group_by(SITE, ACTIVE_YN, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, WORKQUEUE_NAME, WORKQUEUE_ID) %>%
  summarise(total = n()) %>%
  mutate(MSHS = "MSHS") %>%
...
reactable(
  ...
  groupBy = c("MSHS", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "WORKQUEUE_NAME"),

  columnGroups = list(
    colGroup(name = "", columns = c("MSHS", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "WORKQUEUE_NAME", "WORKQUEUE_ID"),
             headerStyle = list(color = "black", fontSize = "15px"),
             sticky = "left"),
    colGroup(name = "# of WQs", columns = c("Y", "N", "total", "deactivated_perc"),
             headerStyle = list(color = "#d80b8c", fontSize = "15px"))
    # colGroup(name = "Visits Arrived/Scheduled in the Last 3 Months", columns = c("total_vol", "new_pts", "new_access", "new_14access_perc"),
    #           headerStyle = list(color = "#00aeef", fontSize = "15px"))
    ),
